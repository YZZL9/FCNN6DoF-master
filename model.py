...
class GroupConv2D(KL.Conv2D):
    ...
    def build(self, input_shape):
        ...
        kernel_shape = kernel_shape.astype('int32')
    ...
    def call(self, inputs):
        ...
        filter_in_group = int(filter_in_group)
        if self.data_format == 'channels_first':
            channel_axis = 1
            ...
            channel_axis = -1
            input_in_group = self.channel_num / self.num_group
            input_in_group = int(input_in_group)
            for i in range(self.num_group):
                outputs = K.conv2d(
                    inputs[:, :, :, i * input_in_group:(i + 1) * input_in_group],
                    self.kernel[:, :, :, i * filter_in_group:(i + 1) * filter_in_group],
        ...
        outputs = KL.concatenate(outputs_list, axis=channel_axis)
        return outputs

    def get_config(self):
        config = super(KL.Conv2D, self).get_config()
        config.pop('rank')
...
def Conv2d_BN(x, nb_filter, kernel_size, strides=(1, 1), padding='same'):
    x = KL.Conv2D(nb_filter, kernel_size, strides=strides, padding=padding)(x)
    x = KL.BatchNormalization(axis=3)(x)
    x = KL.LeakyReLU(alpha=0.1)(x)
    return x
def Conv2dT_BN(x, filters, kernel_size, strides=(2, 2), padding='same'):
    x = KL.Conv2DTranspose(filters, kernel_size, strides=strides, padding=padding)(x)
    x = KL.BatchNormalization(axis=3)(x)
    x = KL.LeakyReLU(alpha=0.1)(x)
    return x
def U_net(input_image,architecture):
    assert architecture in ["resnet50", "resnet101"]
    conv1 = Conv2d_BN(input_image, 8, (3, 3))
    conv1 = Conv2d_BN(conv1, 8, (3, 3))
    pool1 = KL.MaxPooling2D(pool_size=(2, 2), strides=(2, 2), padding='same')(conv1)

    conv2 = Conv2d_BN(pool1, 16, (3, 3))
    conv2 = Conv2d_BN(conv2, 16, (3, 3))
    pool2 = KL.MaxPooling2D(pool_size=(2, 2), strides=(2, 2), padding='same')(conv2)

    conv3 = Conv2d_BN(pool2, 32, (3, 3))
    conv3 = Conv2d_BN(conv3, 32, (3, 3))
    pool3 = KL.MaxPooling2D(pool_size=(2, 2), strides=(2, 2), padding='same')(conv3)

    conv4 = Conv2d_BN(pool3, 64, (3, 3))
    conv4 = Conv2d_BN(conv4, 64, (3, 3))
    pool4 = KL.MaxPooling2D(pool_size=(2, 2), strides=(2, 2), padding='same')(conv4)
    ...
    concat1 = KL.concatenate([conv4, convt1], axis=3)
    concat1 = KL.Dropout(0.5)(concat1)
    conv6 = Conv2d_BN(concat1, 64, (3, 3))
    conv6 = Conv2d_BN(conv6, 64, (3, 3))

    convt2 = Conv2dT_BN(conv6, 32, (3, 3))
    concat2 = KL.concatenate([conv3, convt2], axis=3)
    concat2 = KL.Dropout(0.5)(concat2)
    conv7 = Conv2d_BN(concat2, 32, (3, 3))
    conv7 = Conv2d_BN(conv7, 32, (3, 3))

    convt3 = Conv2dT_BN(conv7, 16, (3, 3))
    concat3 = KL.concatenate([conv2, convt3], axis=3)
    concat3 = KL.Dropout(0.5)(concat3)
    conv8 = Conv2d_BN(concat3, 16, (3, 3))
    conv8 = Conv2d_BN(conv8, 16, (3, 3))

    convt4 = Conv2dT_BN(conv8, 8, (3, 3))
    concat4 = KL.concatenate([conv1, convt4], axis=3)
    concat4 = KL.Dropout(0.5)(concat4)
    conv9 = Conv2d_BN(concat4, 8, (3, 3))
    conv9 = Conv2d_BN(conv9, 8, (3, 3))
    conv9 = KL.Dropout(0.5)(conv9)
    conv9 = Conv2D(filters=3, kernel_size=(1, 1), strides=(1, 1), padding='same', activation='sigmoid')(conv9)
...
def resnet_graph(input_image, architecture, stage5=False):
    ...
    C2 = x = identity_block(x, 3, [64, 64, 256], stage=2, block='c')  #160*160
    # C2 = KL.Add(name='C1C2_ADD')([C2,KL.Conv2D(256,(1,1))(C1)])
    # Stage 3
...
def detection_targets_graph(proposals, gt_boxes, gt_masks, gt_coords, config):
    ...
    gt_masks: [height, width, MAX_GT_INSTANCES] of boo  lean type.
    ...
    class_ids: [TRAIN_ROIS_PER_IMAGE]. Integer class I  Ds. Zero padded.
    deltas: [TRAIN_ROIS_PER_IMAGE, NUM_CLASSES, (dy, d  x, log(dh), log(dw))]
            Class-specific bbox refinments. 
    masks: [TRAIN_ROIS_PER_IMAGE, height, width]. Mask  s cropped to bbox
           boundaries and resized to neural network ou  tput size.f
    coords: [TRAIN_ROIS_PER_IMAGE, height, width, 3].   Coordinate maps cropped to bbox
           boundaries and resized to neural network ou  tput size.
...
class DetectionTargetLayer(KE.Layer):
    ...
    def compute_output_shape(self, input_shape):
        return [
            ...
            (None, self.config.TRAIN_ROIS_PER_IMAGE, self.config.COORD_SHAPE[0], self.config.COORD_SHAPE[1]),
            # coordinate_x
            (None, self.config.TRAIN_ROIS_PER_IMAGE, self.config.COORD_SHAPE[0], self.config.COORD_SHAPE[1]),
            # coordinate_y
            (None, self.config.TRAIN_ROIS_PER_IMAGE, self.config.COORD_SHAPE[0], self.config.COORD_SHAPE[1])
...
def build_fpn_mask_graph(rois, feature_maps,
    ...
    # Conv layers1
    x = KL.TimeDistributed(KL.Conv2D(256, (3, 3), padding="same"),
                           name="mrcnn_mask_conv1{}".format(net_ext))(begin_x)
    if use_bn:
        x = KL.TimeDistributed(BatchNorm(axis=-1),
                               name='mrcnn_mask_bn1{}'.format(net_ext))(x)
    x = KL.Activation('relu')(x)
    ...
    #conv2
    x = KL.TimeDistributed(KL.Conv2D(256, (3, 1), padding="same"),
                           name="mrcnn_mask_conv5{}".format(net_ext))(begin_x)
    if use_bn:
        x = KL.TimeDistributed(BatchNorm(axis=-1),
                               name='mrcnn_mask_bn5{}'.format(net_ext))(x)
    x = KL.Activation('relu')(x)

    x = KL.TimeDistributed(KL.Conv2D(256, (1, 3), padding="same"),
                           name="mrcnn_mask_conv6{}".format(net_ext))(x)
    if use_bn:
        x = KL.TimeDistributed(BatchNorm(axis=-1),
                               name='mrcnn_mask_bn6{}'.format(net_ext))(x)
    x = KL.Activation('relu')(x)

    x = KL.TimeDistributed(KL.Conv2D(256, (3, 3), padding="same"),
                           name="mrcnn_mask_conv7{}".format(net_ext))(x)
    if use_bn:
        x = KL.TimeDistributed(BatchNorm(axis=-1),
                               name='mrcnn_mask_bn7{}'.format(net_ext))(x)
    ...
    feature_x = KL.TimeDistributed(KL.Conv2DTranspose(256, (2,2), strides=2, activation="relu"),
                           name="mrcnn_mask_deconv{}".format(net_ext))(add_x)
    feature_x1 = KL.TimeDistributed(KL.Conv2DTranspose(256, (2,2), strides=2, activation="relu"),
                           name="mrcnn_mask_deconv1{}".format(net_ext))(x1)
    feature_x2 = KL.TimeDistributed(KL.Conv2DTranspose(256, (2,2), strides=2, activation="relu"),
                           name="mrcnn_mask_deconv2{}".format(net_ext))(x2)
    feature_x = KL.add([feature_x,feature_x1,feature_x2])
    x = KL.TimeDistributed(KL.Conv2D(num_classes, (1, 1), strides=1, activation="sigmoid"),
                           name="mrcnn_mask{}".format(net_ext))(feature_x)
    return x, feature_x
...
def build_fpn_mask_coords_graph(rois, feature_maps,
    ...
    x = KL.Lambda(lambda t: tf.reshape(t,
                                       [tf.shape(t)[0], tf.shape(t)[1], tf.shape(t)[2], tf.shape(t)[3], -1, 4]),
                  name="mrcnn_coord")(x)
...
def build_fpn_coords_bins_delta_graph(rois, feature_maps,
    ...
    x1 = KL.Lambda(lambda t: tf.reshape(t,
                                        [tf.shape(t)[0], tf.shape(t)[1], tf.shape(t)[2], tf.shape(t)[3], -1, 3,
                                         num_bins]), name="mrcnn_coord_bins_reshape")(x1)
...
def build_fpn_coord_bins_graph(rois, feature_maps,
    ...
    x = KL.TimeDistributed(KL.Conv2D(256, (3, 3), padding="same"),
                           name="mrcnn_{}_conv2".format(net_name))(x1)
    if use_bn:
        x = KL.TimeDistributed(BatchNorm(axis=-1),
                               name='mrcnn_{}_bn2'.format(net_name))(x)
    x = KL.Activation('relu')(x)
    ...
    x = KL.TimeDistributed(KL.Conv2D(256, (3, 3), padding="same"),
                           name="mrcnn_{}_conv3".format(net_name))(x2)
    if use_bn:
        x = KL.TimeDistributed(BatchNorm(axis=-1),
                               name='mrcnn_{}_bn3'.format(net_name))(x)
    x = KL.Activation('relu')(x)
    ...
    x = KL.TimeDistributed(KL.Conv2D(256, (3, 3), padding="same"),
                           name="mrcnn_{}_conv4".format(net_name))(x3)
    if use_bn:
        x = KL.TimeDistributed(BatchNorm(axis=-1),
                               name='mrcnn_{}_bn4'.format(net_name))(x)
    x = KL.Activation('relu')(x)
...
def build_fpn_coord_bins_delta_graph(rois, feature_maps,
    ...
    """Builds the computation graph offered the coordinate map head of Feature Pyramid Network.
    ...
    x1 = KL.Lambda(lambda t: tf.reshape(t,
                   ...
                   name="mrcnn_{}_bins_reshape".format(net_name))(x1)
...
def mrcnn_coord_symmetry_loss_graph(target_masks, target_coords, target_class_ids, target_domain_labels, pred_coords,
    ...
    def nonzero_positive_loss(target_masks, target_coords, pred_coords, positive_ix):
        ...
        mean_diff_in_mask = tf.divide(min_diff_in_mask,
                                      tf.expand_dims(num_of_pixels, axis=1))  ## shape: [num_pos_rois, 3]
...
def mrcnn_coord_symmetry_euclidean_distance_graph(target_masks, target_coords, target_class_ids, target_domain_labels,
                                                  pred_coords):
...
def mrcnn_coord_bins_symmetry_loss_graph(target_masks, target_coords, target_class_ids, target_domain_labels,
                                         pred_coords):
    ...
    def nonzero_positive_loss(target_masks, target_coords, pred_coords_trans, positive_ix):
        ...
        mean_loss_in_mask = tf.divide(min_loss_in_mask,
                                      tf.expand_dims(num_of_pixels, axis=1))  ## shape: [num_pos_rois, 3]
...
#new
#########################################################
class D_conv(keras.layers.Layer):
    def __init__(self):
        super(D_conv,self).__init__()
    def call(self,inputs):
        filters = tf.constant(value=1, shape=[3, 3, 256, 256], dtype=tf.float32)
        output = tf.nn.atrous_conv2d(value=inputs, filters=filters, rate=2, padding="SAME")
        return  output
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        # CBAM
        cbam_P2 = cbam_block(P2)
        cbam_P3 = cbam_block(P3)
        cbam_P4 = cbam_block(P4)
        cbam_P5 = cbam_block(P5)
        P5 = KL.add([P5,cbam_P5])
        P4 = KL.add([P4,cbam_P4])
        P3 = KL.add([P3,cbam_P3])
        P2 = KL.add([P2,cbam_P2])
            ...
            if config.COORD_USE_BINS:
                ...
                if config.USE_SYMMETRY_LOSS:
                    ...
                    coord_bin_loss = KL.Lambda(lambda x: mrcnn_coord_bins_symmetry_loss_graph(*x),
                                               name="mrcnn_coord_bin_loss")(
                        [target_mask, target_coords, target_class_ids,
                ...
                #不走这里
                    ...
                    coord_x_softl1_loss = KL.Lambda(lambda x: mrcnn_coord_smooth_l1_loss_graph(*x),
                                                    name="mrcnn_coord_x_l1_loss")(
                        [target_mask, target_coord_x, target_class_ids, mrcnn_coord_x_bin])
                    coord_y_softl1_loss = KL.Lambda(lambda x: mrcnn_coord_smooth_l1_loss_graph(*x),
                                                    name="mrcnn_coord_y_l1_loss")(
                        [target_mask, target_coord_y, target_class_ids, mrcnn_coord_y_bin])
                    coord_z_softl1_loss = KL.Lambda(lambda x: mrcnn_coord_smooth_l1_loss_graph(*x),
                                                    name="mrcnn_coord_z_l1_loss")(
                        [target_mask, target_coord_z, target_class_ids, mrcnn_coord_z_bin])
                ...
                if config.COORD_USE_DELTA:
                    ...
                    if config.USE_SYMMETRY_LOSS:
                        ...
                        coord_x_delta_loss = KL.Lambda(lambda x: tf.reshape(x[0], (1, 1)),
                                                       name="mrcnn_coord_x_delta_loss")(coord_loss)
                        coord_y_delta_loss = KL.Lambda(lambda x: tf.reshape(x[1], (1, 1)),
                                                       name="mrcnn_coord_y_delta_loss")(coord_loss)
                        coord_z_delta_loss = KL.Lambda(lambda x: tf.reshape(x[2], (1, 1)),
                                                       name="mrcnn_coord_z_delta_loss")(coord_loss)
                    else:
                    ...
                    mrcnn_coord_x = KL.Lambda(lambda x: tf.identity(x), name="mrcnn_coord_x_final")(
                        mrcnn_coord_x_bin_value)
                    mrcnn_coord_y = KL.Lambda(lambda x: tf.identity(x), name="mrcnn_coord_y_final")(
                        mrcnn_coord_y_bin_value)
                    mrcnn_coord_z = KL.Lambda(lambda x: tf.identity(x), name="mrcnn_coord_z_final")(
                        mrcnn_coord_z_bin_value)
        ...
        #程序不走这里
            ...
            if config.COORD_USE_BINS:
                ...
                ## regress the coordinate map with shared weights
                if config.COORD_SHARE_WEIGHTS:
                    mrcnn_coord_x, mrcnn_coord_y, mrcnn_coord_z, mrcnn_coord_feature = build_fpn_coord_graph(
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers_name):
        ...
        config_output_path = os.path.join(self.log_dir,
                                          "training_config_layer_{}_epoch_{}.txt".format(layers_name, epochs))
